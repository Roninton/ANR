...
class HypoNerf(nn.Module):
    ...
    def __init__(self, use_viewdirs=False, depth=6, hidden_dim=256, use_pe=True, pe_sigma=1024, pe_dim=40):
        super().__init__()
    ...
    def convert_posenc(self, x):
        w = 2**torch.linspace(0, np.log(self.pe_sigma), self.pe_dim // 2, device=x.device)
